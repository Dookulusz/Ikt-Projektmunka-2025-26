var alkatrészek = {
    //AMD PROCESSZOROK: 
    ryzen99950x3d: {
        kep: '../Dookusz/Kepek/Processzor/Ryzen/Ryzen 9 9950x3d.png',
        nev: 'Ryzen 9 9950X3D',
        ar: '269 990 Ft'
    },

    ryzen99950x: {
        kep: '../Dookusz/Kepek/Processzor/Ryzen/Ryzen 9 9950x.png',
        nev: 'Ryzen 9 9950X',
        ar: '207 650 Ft'
    },

    ryzen99900X3D: {
        kep: '../Dookusz/Kepek/Processzor/Ryzen/Ryzen 9 9900x3d.jpg',
        nev: 'Ryzen 9 9900X3D',
        ar: '226 200 Ft'
    },

    ryzen99900x: {
        kep: '../Dookusz/Kepek/Processzor/Ryzen/Ryzen 9 9900x3d.png',
        nev: 'Ryzen 9 9900X',
        ar: '152 941 Ft'
    },

    ryzen97950x: {
        kep: '../Dookusz/Kepek/Processzor/Ryzen/Ryzen 9 7950x.png',
        nev: 'Ryzen 9 7950X',
        ar: '154 700 Ft'
    },

    //AMD ÉGE

    //INTEL
    inteli914900k: {
        kep: '../Dookusz/Kepek/Processzor/Intel/i914900k.jpg',
        nev: 'Intel Core i9-14900K',
        ar: '183 225 Ft'
    },

    inteli914900kf: {
        kep: '../Dookusz/Kepek/Processzor/Intel/i9-1400ks.jpg',
        nev: 'Intel Core i9-14900KF',
        ar: '178 600 Ft'
    },

    inteli914900ks: {
        kep: '../Dookusz/Kepek/Processzor/Intel/i9-1400ks.jpg',
        nev: 'Intel Core i9-14900KS',
        ar: '263 900 Ft'
    },

    inteli714700k: {
        kep: '../Dookusz/Kepek/Processzor/Intel/i7.jpg',
        nev: 'Intel Core i7-14700K',
        ar: '138 500 Ft'
    },

    inteli714700kf: {
        kep: '../Dookusz/Kepek/Processzor/Intel/i7.jpg',
        nev: 'Intel Core i7-14700KF',
        ar: '128 100 Ft'
    },

    // Intel vége
};

var procinevek = [
    "ryzen99950x3d",
    "ryzen99950x",
    "ryzen99900X3D",
    "ryzen99900x",
    "ryzen97950x",
    "inteli914900k",
    "inteli914900kf",
    "inteli914900ks",
    "inteli714700k",
    "inteli714700kf",
];

var alkatrészek1 = {
    //Alaplap

    gigab650: {
        kep: '../Dookusz/Kepek/Alaplap/1-es.jpg',
        nev: 'GIGABYTE B650 EAGLE AX',
        ar: '55 976 Ft',
    },

    msimagz890w: {
        kep: '../Dookusz/Kepek/Alaplap/2-es.jpg',
        nev: 'MSI MAG Z890 TOMAHAWK WIFI',
        ar: '114 491 Ft',
    },

    asrockb450: {
        kep: '../Dookusz/Kepek/Alaplap/3-as.jpg',
        nev: 'ASRock B450M-HDV R4.0',
        ar: '24 290 Ft',
    },

    asustufga520: {
        kep: '../Dookusz/Kepek/Alaplap/4-es.jpg',
        nev: 'ASUS TUF Gaming A520M-PLUS II',
        ar: '26 890 Ft',
    },

    msib760gpw: {
        kep: '../Dookusz/Kepek/Alaplap/5-es.jpg',
        nev: 'MSI B760 GAMING PLUS WIFI DDR4',
        ar: '42 990 Ft',
    }


};

var alaplapnevek = [
    "gigab650",
    "msimagz890w",
    "asrockb450",
    "asustufga520",
    "msib760gpw",
];


var alkatrészek2 = {
    //storage
    kinston1tb: {
        kep: '../Dookusz/Kepek/Tárhely/1-es.jpg',
        nev: 'Kingston NV3 1TB (SNV3S/1000G)',
        ar: '39 990 Ft',
    },

    samsung25: {
        kep: '../Dookusz/Kepek/Tárhely/2-es.jpg',
        nev: 'Samsung 2.5 870 EVO 1TB SATA3 (MZ-77E1T0B)',
        ar: '44 990 Ft',
    },

    samsung990: {
        kep: '../Dookusz/Kepek/Tárhely/3-as.jpg',
        nev: 'Samsung 990 PRO 2TB M.2 (MZ-V9P2T0BW)',
        ar: '103 378 Ft',
    },

    weste1tb: {
        kep: '../Dookusz/Kepek/Tárhely/4-es.jpg',
        nev: 'Western Digital WD Black SN7100 1TB M.2 (WDS100T4X0E)',
        ar: '39 990 Ft',
    },

    kingstona400: {
        kep: '../Dookusz/Kepek/Tárhely/5-es.jpg',
        nev: 'Kingston A400 2.5 960GB SATA3 (SA400S37/960G)',
        ar: '36 999 Ft',
    },

};

storagenevek = [
    "kinston1tb",
    "samsung25",
    "samsung990",
    "weste1tb",
    "kingstona400",
];

alkatrészek3 = {
    bequiet850: {
        kep: '../Dookusz/Kepek/Tápegység/1-es.jpg',
        nev: 'be quiet! Pure Power 13 M 850W',
        ar: '55 643 Ft',
    },

    corsair650: {
        kep: '../Dookusz/Kepek/Tápegység/2-es.jpg',
        nev: 'Corsair RMe 2025 RM650e 80 PLUS Gold 650W',
        ar: '33 090 Ft',
    },

    fspvitan: {
        kep: '../Dookusz/Kepek/Tápegység/3-as.jpg',
        nev: 'FSP VITA GM 850W 80 PLUS Gold ',
        ar: '46 890 Ft',
    },
    coolermaster850: {
        kep: '../Dookusz/Kepek/Tápegység/4-es.jpg',
        nev: 'Cooler Master MWE 850W v3 80 PLUS Gold',
        ar: '33 890 Ft',
    },
    deepcool750: {
        kep: '../Dookusz/Kepek/Tápegység/5-es.jpg',
        nev: 'Deepcool PF750 750W',
        ar: '22 610 Ft',
    },
};

psunevek = [
    "bequiet850",
    "corsair650",
    "fspvitan",
    "coolermaster850",
    "deepcool750",
];

alkatrészek5 = {
    nzxt1: {
        kep: '../Dookusz/Kepek/Gépház/1-es.jpg',
        nev: 'NZXT H7 Flow 2024 All Black',
        ar: '48 190 Ft',
    },
    nzxt2: {
        kep: '../Dookusz/Kepek/Gépház/2-es.jpg',
        nev: 'NZXT H9 Elite White',
        ar: '70 190 Ft',
    },
    linalicsung: {
        kep: '../Dookusz/Kepek/Gépház/3-as.jpg',
        nev: 'Lian Li Lancool 217 Wood-Black',
        ar: '44 450 Ft',
    },
    asuspccase: {
        kep: '../Dookusz/Kepek/Gépház/4-es.jpg',
        nev: 'ASUS A31 Plus TG ARGB Black',
        ar: '32 450 Ft',
    },
    bequietcase: {
        kep: '../Dookusz/Kepek/Gépház/5-es.jpg',
        nev: 'be quiet! Pure Base 500DX Black',
        ar: '48 410 Ft',
    },
};

casenevek = [
    "nzxt1",
    "nzxt2",
    "linalicsung",
    "asuspccase",
    "bequietcase",
];

var alkatrészek4 = {
    rtx5070ti: {
        kep: '../Dookusz/Kepek/Videokártya/1-es.jpg',
        nev: 'ASUS GeForce RTX 5070 Ti PRIME OC 16GB GDDR7 256bit',
        ar: '339 900 Ft',
    },

    rx9060xt: {
        kep: '../Dookusz/Kepek/Videokártya/2-es.jpg',
        nev: 'SAPPHIRE Radeon RX 9060 XT Pulse OC 16GB GDDR6 128bit',
        ar: '157 770 Ft',
    },

    rx9070xt: {
        kep: '../Dookusz/Kepek/Videokártya/3-as.jpg',
        nev: 'SAPPHIRE Radeon RX 9070 XT PULSE 16GB GDDR6 256bit',
        ar: '314 790 Ft',
    },

    rtx5080: {
        kep: '../Dookusz/Kepek/Videokártya/4-es.jpg',
        nev: 'GIGABYTE GeForce RTX 5080 WINDFORCE OC SFF 16GB GDDR7 256bit',
        ar: '435 990 Ft',

    },

    rtx5090: {
        kep: '../Dookusz/Kepek/Videokártya/5-es.jpg',
        nev: 'ASUS GeForce RTX 5090 ROG ASTRAL OC 32GB GDDR7 512bit',
        ar: '1 346 990 Ft',
    }
};

var gpunevek = [
    "rtx5070ti",
    "rx9060xt",
    "rx9070xt",
    "rtx5080",
    "rtx5090"
];

var alkatrészek6 = {

    kingstoneram5: {
        kep: '../Dookusz/Kepek/RAM/1-es.jpg',
        nev: 'Kingston FURY Beast 32GB (2x16GB) DDR5 6000MHz',
        ar: '123 490 Ft',
    },

    kingstoneram4: {
        kep: '../Dookusz/Kepek/RAM/2-es.jpg',
        nev: 'Kingston FURY Beast 16GB (2x8GB) DDR4 3200MHz',
        ar: '63 633 Ft',
    },

    kingstoneram5rgb: {
        kep: '../Dookusz/Kepek/RAM/3-as.jpg',
        nev: 'Kingston FURY Beast RGB 32GB (2x16GB) DDR5 6000MHz',
        ar: '129 390 Ft',
    },

    corsairddr5: {
        kep: '../Dookusz/Kepek/RAM/4-es.jpg',
        nev: 'Corsair VENGEANCE RGB 32GB (2x16GB) DDR5 6000MHz',
        ar: '416 900 Ft',
    },

    gskillddr5: {
        kep: '../Dookusz/Kepek/RAM/5-es.jpg',
        nev: 'G.SKILL Trident Z5 Neo RGB 32GB (2x16GB) DDR5 6000MHz',
        ar: '141 390 Ft',
    }


};

var ramnevek = [
    "kingstoneram5",
    "kingstoneram4",
    "kingstoneram5rgb",
    "corsairddr5",
    "gskillddr5"
];

var kiegészítők = {

    kabel: {
        kep: '../Dookusz/Kepek/Kiegészítők/1-es.jpg',
        nev: 'AlzaPower AluCore Premium HDMI 2.0 High Speed 4K, 3m - fekete',
        ar: '4 390 Ft',
    },
    extssd: {
        kep: '../Dookusz/Kepek/Kiegészítők/2-es.jpg',
        nev: 'Seagate Expansion Portable 1TB (2021)',
        ar: '26 590 Ft',
    },
    szek: {
        kep: '../Dookusz/Kepek/Kiegészítők/3-as.jpg',
        nev: 'Rapture NEST fekete',
        ar: '49 290 Ft',
    },
    egbillkomb: {
        kep: '../Dookusz/Kepek/Kiegészítők/4-es.jpg',
        nev: 'RAPOO 8150M Silent',
        ar: '9 499 Ft',
    },
    hang: {
        kep: '../Dookusz/Kepek/Kiegészítők/5-es.jpg',
        nev: 'Creative Pebble V3 fehér',
        ar: '15 190 Ft',
    },
};

var kiegnevek = [
    "kabel",
    "extssd",
    "szek",
    "egbillkomb",
    "hang",
];

function valtoztatas(tömb, lista, cím) {

    for (var i = 0; i < lista.length; i++) {
        var listanev = lista[i]
        var pocok = tömb[listanev]

        var index = i + 1;

        var kep = document.getElementById('kep' + index)
        var nev = document.getElementById('nev' + index)
        var ar = document.getElementById('ar' + index)

        if (kep) kep.src = pocok.kep
        if (nev) nev.innerHTML = pocok.nev
        if (ar) ar.innerHTML = pocok.ar
    }

    for (var i = 0; i <= 10; i++) {
        if (i > 5 && tömb != alkatrészek) {
            document.getElementById(i).style.display = "none"
        }
        else if (tömb == alkatrészek && i > 5) {
            document.getElementById(i).style.display = ""
        }
    }

    document.getElementById("hedvig").innerHTML = cím
}

function proci() {
    var cim = "Processzorok";
    valtoztatas(alkatrészek, procinevek, cim)
}

function alaplap() {
    var cim = "Alaplapok";
    valtoztatas(alkatrészek1, alaplapnevek, cim)
}

function storage() {
    var cim = "Háttértárak";
    valtoztatas(alkatrészek2, storagenevek, cim);
}

function psu() {
    var cim = "Tápegységek";
    valtoztatas(alkatrészek3, psunevek, cim);
}

function gephaz() {
    var cim = "Gépházak";
    valtoztatas(alkatrészek5, casenevek, cim);
}

function gpu() {
    var cim = "Videókártyák";
    valtoztatas(alkatrészek4, gpunevek, cim);
}

function ram() {
    var cim = "Memória";
    valtoztatas(alkatrészek6, ramnevek, cim);
}

function accessories() {
    var cim = "Kiegészítők";
    valtoztatas(kiegészítők, kiegnevek, cim);
}

var darab = 0;

function gomb() {
    darab = darab + 1;
    window.alert("Termék hozzáadása a kosárba");
    document.getElementById("termek").innerHTML = "Kosár: " + darab;
}

//A trim() eltávolítja a szöveg eleji és végi fölösleges szóközöket :)
function regist() {
    var email = document.getElementById("emailem").value.trim()
    var jelszo = document.getElementById("jelszom").value.trim()
    var nev = document.getElementById("nevem").value.trim()

    if (email !== "" && email.includes("@") && jelszo.length >= 4) {
        localStorage.setItem("email", email)
        localStorage.setItem("jelszo", jelszo)
        localStorage.setItem("nev", nev)

        window.alert("Sikeres regisztráció!")
        setTimeout(function () {
            window.location.href = "../Dookusz/bejelentkezes.html"
        }, 100); // 100ms várakozás
    } else {
        window.alert("Hiba")
    }
}

//Üdvözlök mindenkit!

//Elég nagy poén az hogy itt nézem a kódot hogy mi a baj hogy nem megy a bejelentkezés és kiderült
//hogy az volt a gond hogy nem ugyanazt írtam be kétszer
//én meg csodálkoztam hogy miért nem működik XD! (+1 indok hogy miért nem jó betegen kódolni :) )

//A localstorage nem az én művem viszont maga az ötlet igen. 
function bejelentekez() {
    var emailertek = document.getElementById("emailem1").value.trim()
    var jelszoertek = document.getElementById("jelszom1").value.trim()

    var emailes = localStorage.getItem("email")
    var jelszos = localStorage.getItem("jelszo")

    if (!emailes || !jelszos) {
        window.alert("Nincs regisztrált fiókja!")
        return
        // A return azért kell nekem hogy ne írja ki a nincs fiók
        //  után egybőé azt hogy sikertelen bejelentkezés :)
    }

    if (emailertek === emailes && jelszoertek === jelszos) {
        window.alert("Sikeres bejelentkezés!");
        setTimeout(function () {
            window.location.href = "../Dookusz/kosar.html"
        }, 100); // 100ms várakozás
    }
    else {
        window.alert("Sikertelen bejelentkezés")
    }
}

function kijelentkez() {
    //ez a localstorage minden elemét törli (sajnos ha kijelentkezünk akkor újra kell regisztrálni de most ez van :/)
    localStorage.clear()
    document.getElementById("fiokletrehoz").style.display = ""
    document.getElementById("Bejelenkez").innerHTML = "Bejelentkezés"
    document.getElementById("kijelenkez").style.display = "none"

}

document.getElementById("kijelenkez").style.display = "none"

window.addEventListener("DOMContentLoaded", function () {
    var email = localStorage.getItem("email")
    var jelszo = localStorage.getItem("jelszo")
    var nev = localStorage.getItem("nev")

    // Csak akkor fut le ha van bejelentkezett felhasználó
    if (email && jelszo && nev) {
        document.getElementById("Bejelenkez").innerHTML = "Üdvözöljük: " + nev
        document.getElementById("fiokletrehoz").style.display = "none"
        document.getElementById("kijelenkez").style.display = ""
    }
})

function megrendel() {
    var email = localStorage.getItem("email")
    var jelszo = localStorage.getItem("jelszo")
    var nev = localStorage.getItem("nev")

    //csak akkor tudunk rendelni terméket ha be van jelentkezve a felhasználó
    if (email && jelszo && nev) {
        alert("Köszönjük a rendelését!")
        setTimeout(function () {
            window.location.href = "../Borka/fooldal.html"
        }, 100)
    }
    else {
        alert("A rendeléshez be kell jelentkezni!")
    }
}

//Kérdés/kérés esetén Outlookon meg lehet találni, mivel a héten nem vagyok! :)